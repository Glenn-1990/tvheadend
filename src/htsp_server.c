...
htsp_build_dvrentry(dvr_entry_t *de, const char *method)
{
  ...
  if (de->de_autorec)
    htsmsg_add_str(out, "autorecId", de->de_autorec->dae_id);
...
htsp_build_autorecentry(dvr_autorec_entry_t *dae, const char *method)
{
  ...
  htsmsg_add_str(out, "id", dae->dae_id);
...
htsp_method_async(htsp_connection_t *htsp, htsmsg_t *in)
{
  ...
  /* Send all autorec entries */
  TAILQ_FOREACH(dae, &autorec_entries, dae_global_link)
  {
    tvhlog(LOG_INFO, "htsp", "--------------add autorec %s id:%s", dae->dae_title, dae->dae_id);
...
htsp_method_addAutorecEntry(htsp_connection_t *htsp, htsmsg_t *in)
{
  ...
  dae = dvr_autorec_add(dvr_config_name, title,
                    ch, NULL, NULL, min_duration, max_duration, approx_time,
                    days_of_week, priority, retention, start_extra, stop_extra, creator, comment);
...
htsp_autorec_entry_delete(dvr_autorec_entry_t *dae)
{
  ...
  htsmsg_t *m = htsmsg_create_map();
  htsmsg_add_str(m, "id", dae->dae_id);
